...
class LearnerGPGTrainer(GPGTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.sync_weights_path = Path(
            os.getenv("SYNC_WEIGHTS_PATH", "/extrahome0/save_dir/GPG/4gpus/Async/tmp/Qwen3-0.6B/gpg_async_weights.pt"))
        ...
        # 从环境变量或参数中获取超时时间
        # 默认设置为10分钟，以防采样器暂时卡顿
        self.queue_timeout = int(os.getenv("QUEUE_TIMEOUT_SECONDS", "600"))
...
def main(script_args, training_args, model_args):
    ...
    # 添加用于模型同步的回调
    sync_weights_path = Path(os.getenv("SYNC_WEIGHTS_PATH", "/extrahome0/save_dir/GPG/4gpus/Async/tmp/Qwen3-0.6B/gpg_async_weights.pt"))
