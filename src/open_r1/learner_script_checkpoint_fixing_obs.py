...
from open_r1.configs_obs import GRPOConfig, GRPOScriptArguments, GPGConfig
...
#################################################################################################################
from async_utils_checkpoint_fixing_obs import split_tensor_dict, setup_fs_queue, pop_from_fs_queue, SamplerSyncCallback # 新增
#################################################################################################################
...
class Learner_MoISTrainer(Trainer):
    ...
    def __init__(
        ...
        self.obs_bucket_name = self.obs_queue_dir[6:].split('/', 1)[0]
    ...
    def _prepare_inputs(
        ...
        mode = "train" if self.model.training else "eval"
        if mode == "train":
            # generate_every = self.args.steps_per_generation * self.num_iterations # steps_per_generation=8 可以简单看成等于梯度累加数, num_iterations=1, 迭代次数，不会发生变化，generate_every=8
            assert self.args.steps_per_generation * self.num_iterations % self.args.world_size == 0, f"the number of GPUs should be able to be divided by {self.args.steps_per_generation * self.num_iterations}, but now it is {self.args.world_size}"
            generate_every = self.args.steps_per_generation * self.num_iterations // self.args.world_size
            if self._step % generate_every == 0 or self._buffered_inputs is None: # _step = 0
                
                # self._buffered_inputs=None can occur when resuming from a checkpoint
                generation_batch = self._generate_and_score_completions(generation_batch) # 64 -> 16
                generation_batch = shuffle_tensor_dict(generation_batch) # 64 -> 16
                # self._buffered_inputs = split_tensor_dict(generation_batch, self.args.steps_per_generation) # 8 -> 2
                self._buffered_inputs = split_tensor_dict(generation_batch, self.args.steps_per_generation // self.args.world_size)
            inputs = self._buffered_inputs[self._step % (self.args.steps_per_generation // self.args.world_size)]
            self._step += 1
        else:
