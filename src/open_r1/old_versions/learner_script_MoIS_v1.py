...
class LearnerGPGTrainer(GPGTrainer):
    ...
    def get_GPG_loss(self, inputs, learner_per_token_logps):
        advantages = inputs["advantages"]
        per_token_loss_GPG = - learner_per_token_logps * advantages.unsqueeze(1)
        return per_token_loss_GPG


    def get_loss_binary(self, inputs, learner_per_token_logps,threhold=8):
        # Compute the loss
        per_token_loss_GRPO = self.get_GRPO_loss(inputs, learner_per_token_logps)
        per_token_loss_GPG  = self.get_GPG_loss(inputs, learner_per_token_logps)

        mode = "train" if self.model.training else "eval"
        model_ids = inputs['metrics'][mode]['model_ids'][-1]
        step_diff = self.state.global_step - model_ids
        assert step_diff >=0, f"{self.state.global_step}-{model_ids} should be > 0"
        lambdaT = 1.0 if step_diff <= threhold else 0.0
        per_token_loss = lambdaT * per_token_loss_GPG + (1 - lambdaT) * per_token_loss_GRPO
        return per_token_loss

    def get_loss_mix(self, inputs, learner_per_token_logps):
    ...
    def get_loss_MoIS(self, inputs, learner_per_token_logps):
        def compute_lambdaT(step_diff, threhold=8, k=1):
            """计算动态权重 lambdaT"""
            return 1 / (1 + np.exp(k * (step_diff - threhold)))
        mode = "train" if self.model.training else "eval"
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        learner_per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, logits_to_keep)


        per_token_loss = self.get_loss_MoIS(inputs, learner_per_token_logps)

        
        #Log the metrics
        mode = "train" if self.model.training else "eval"

        # KL损失 Compute the KL divergence between the model and the reference model
        if self.beta != 0.0:
            ref_per_token_logps = inputs["ref_per_token_logps"]
            per_token_kl = (
                torch.exp(ref_per_token_logps - learner_per_token_logps) - (ref_per_token_logps - learner_per_token_logps) - 1
            )
