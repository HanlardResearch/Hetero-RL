...
readme="""
基于6b版本，把avg-logp 换成 log p 保留 防御性IS
"""
############################################
    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.loss_type in ["grpo", "bnpo","dr_grpo"]:
            ...
            if self.loss_type == "mois":
                ...
                coef_1 = learner_seq_p / (sampler_seq_p*(1-std_sampler_seq_p) + learner_seq_p.detach() * std_sampler_seq_p)
                coef_2 = torch.clamp(coef_1, 1 - self.epsilon_low, 1 + self.epsilon_high)
                ...
                # weight1 = learner_seq_p.detach() / (avg_sampler_seq_p*(1-std_sampler_seq_p) + std_sampler_seq_p * learner_seq_p.detach() )
                # weight2 = learner_seq_p.detach() / sampler_seq_lopp.exp().detach()
                # weight3 = learner_seq_p.detach() / avg_sampler_seq_p.detach()
                #
                # ESS1 = get_ESS(weight1)
                # ESS2 = get_ESS(weight2)
                # ESS3 = get_ESS(weight3)
                #
                # self._metrics[mode]["ESS1"].append(ESS1)
                # self._metrics[mode]["ESS2"].append(ESS2)
                # self._metrics[mode]["ESS3"].append(ESS3)
