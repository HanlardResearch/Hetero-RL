...
class Learner_MoISTrainer(Trainer):
    ...
    def _compute_loss(self, model, inputs):
        ...
        coef_1 = (learner_seq_lopp-sampler_seq_lopp).exp()
        ...
        low_clip = (is_low_clipped * completion_mask.sum(-1)).sum() / completion_mask.sum()
        high_clip = (is_high_clipped * completion_mask.sum(-1)).sum() / completion_mask.sum()
        clip_ratio = (is_region_clipped * completion_mask.sum(-1)).sum() / completion_mask.sum()
