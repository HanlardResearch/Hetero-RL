...
class Learner_MoISTrainer(Trainer):
    ...
    def _compute_loss(self, model, inputs):
        ...
        epislon = torch.clamp(cppo_per_seq_kl.detach(), 0, 1)

        # $E_q q$ 代替 q
        # E_qP =  (sampler_seq_p * learner_seq_p).sum() /  sampler_seq_p.sum()
        E_qQ =  (sampler_seq_p * sampler_seq_p).sum() /  sampler_seq_p.sum()
        # coef_1 = learner_seq_p / E_qQ
        coef_1 = learner_seq_p / (E_qQ * (1 - std_sampler_seq_p) + std_sampler_seq_p * learner_seq_p.detach())
