...
from async_utils0810 import setup_fs_queue, push_to_fs_queue # 新增
...
class SamplerGRPOTrainer(GRPOTrainer):
    ...
    def generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            sampler_per_token_logps = self._get_per_token_logps(
                self.model, prompt_completion_ids, attention_mask, logits_to_keep, batch_size
            ) if self.loss_type in ["grpo","bnpo", "dr_grpo", "gspo", "EqQ", "EqP"] else None
    ...
    @profiling_decorator
    def _sync_model_weights(self, timeout: int = 600):
        """检查并加载学习器分享的最新模型权重。"""
        try:
