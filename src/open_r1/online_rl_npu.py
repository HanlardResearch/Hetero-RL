...
class OnlineRLTrainer(GRPOTrainer):
    ...
    def _compute_loss(self, model, inputs):
        ...
        ########################## WANDB 显示的统计量 #######################
        self._metrics[mode]["ratio/mean"].append(nanmean(coef_1).item())
        ...
        self._metrics[mode]["ratio_grpo"].append(nanmean(ratio_grpo).item())
        self._metrics[mode]["ratio_pq"].append(nanmean(ratio_gspo).item())
        self._metrics[mode]["ratio_pEqQ"].append(nanmean(ratio_pEqQ).item())
        self._metrics[mode]["ratio_pEqP"].append(nanmean(ratio_pEqP).item())
        ...
        if self.loss_type in ["grpo", "bnpo", "dr_grpo"]:
            ...
            self._metrics[mode]["clip_ratio/low_mean"].append(nanmean(gathered_low_clip).item())
            self._metrics[mode]["clip_ratio/low_min"].append(nanmin(gathered_low_clip).item())
            gathered_high_clip = self.accelerator.gather(high_clip)
            self._metrics[mode]["clip_ratio/high_mean"].append(nanmean(gathered_high_clip).item())
            self._metrics[mode]["clip_ratio/high_max"].append(nanmax(gathered_high_clip).item())
            gathered_clip_ratio = self.accelerator.gather(clip_ratio)
            self._metrics[mode]["clip_ratio/region_mean"].append(nanmean(gathered_clip_ratio).item())
        return loss
