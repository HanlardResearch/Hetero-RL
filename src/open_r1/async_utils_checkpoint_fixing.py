...
def push_to_fs_queue(self, data: Dict[str, Any]):
    ...
    assert data['advantages'].shape[0] % learner_world_size == 0, f"the number of GPUs that learner used should be able to be divided by {data['advantages'].shape[0]}, but now it is {learner_world_size}"
    ...
    for rank in range(learner_world_size):
        ...
        # 这种方式可以防止消费者读到不完整的文件。
        final_filename = f"data_{int(time_save * 1000)}_SamplerRank_{rank}_ModelID_{self.model_ids}_{uuid.uuid4().hex[:6]}.pt"
        final_path = self.queue_dir / final_filename
        # wait_for_everyone()
        os.rename(tmp_path, final_path)
        print(f"文件保存在: {final_path}")
    print(f"rank_{self.rank}已经保存了{learner_world_size}个文件")
