        ...
        if self.loss_type in ["grpo", "bnpo","dr_grpo"]:
            ...
            if self.loss_type == "mois":
                ...
                per_token_loss_stack = torch.stack([
                     per_token_loss1,
                     per_token_loss2,
                     1e8*torch.ones_like(per_token_loss3),
                     per_token_loss4,
                     1e8*torch.ones_like(per_token_loss5),
                ])
                ...
                for loss_id, value_of_per_token_loss_id in enumerate([per_token_loss1,
                                                                      per_token_loss2,
                                                                      per_token_loss3,
                                                                      per_token_loss4,
                                                                      per_token_loss5]):
                    ...
                    # 各个loss的值
                    value_of_loss_id = (-value_of_per_token_loss_id.detach() * completion_mask).sum() / completion_mask.sum()
