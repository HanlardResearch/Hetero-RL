...
def find_ais_track(processed_data_path: Path, data_model_id: int, rank: int, world_size: int = 4):
    ...
    if data_model_id >= processed_max_model_id:
        ...
        id2namelist = {id: [] for id in range(data_model_id + 1, processed_max_model_id + 1)}
        for pt_name in sorted_name_list:
            id =  get_model_id_from_name(pt_name.name)
            if id > data_model_id and id <= processed_max_model_id:
                id2namelist[id].append(pt_name)
        history_advs = []
        aix_track = []
        for id in range(data_model_id + 1, processed_max_model_id + 1):
...
def pop_from_fs_queue(self, queue_dir: Path, processing_dir: Path, rank: int, timeout: int = 600, AIS_len: int = 8, max_diff_step: int = 12) -> Optional[Dict[str, Any]]:
    ...
    while True:
        ...
        # 普通重要性采样
            ...
            history_adv,num_files_of_prcessing,processed_max_model_id = find_ais_track(processed_data_path=processing_dir, data_model_id=queue_dir_max_model_id, rank=rank)
