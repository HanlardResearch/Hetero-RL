...
def variance_EqP(a, b):
    """
    计算 Var_q( p / E_q[p] )
    其中 p = N(b,1), q = N(a,1)
    """

    # 1. 计算 E_q[p(x)] = ∫ p(x) q(x) dx
    def p_q(x):
        p = gaussian_pdf(x, b)
        q = gaussian_pdf(x, a)
        return p * q

    E_q_p, err1 = quad(p_q, -np.inf, np.inf, limit=100, epsabs=1e-6, epsrel=1e-6)

    if E_q_p < 1e-30:
        return np.inf  # 防止除零

    # 2. 计算 E_q[p(x)^2] = ∫ p(x)^2 q(x) dx
    def p_sq_q(x):
        p = gaussian_pdf(x, b)
        q = gaussian_pdf(x, a)
        return (p ** 2) * q

    E_q_p_sq, err2 = quad(p_sq_q, -np.inf, np.inf, limit=100, epsabs=1e-6, epsrel=1e-6)

    # 3. 计算方差
    # Var(w) = E[w^2] - (E[w])^2
    # w = p / E_q[p]
    # E_q[w] = E_q[p] / E_q[p] = 1
    # E_q[w^2] = E_q[p^2] / (E_q[p])^2
    E_w2 = E_q_p_sq / (E_q_p ** 2)
    var = E_w2 - 1.0  # 因为 E[w] = 1

    return max(var, 0)  # 防止数值误差导致负值
...
Var_EqP = np.zeros_like(A)
Var_EqQ = np.zeros_like(A)
...
for i in tqdm(range(A.shape[0])):
    for j in range(A.shape[1]):
        ...
        Var_EqP[i, j] = variance_EqP(a, b)
        Var_EqQ[i, j] = variance_EqQ(a, b)
...
mask = Var_std < Var_EqP

# 将这些点展平，避免空数组报错
A_flat = A[mask]
B_flat = B[mask]
new_var_flat = Var_EqP[mask]
...
# === 新增：标记 new_var > old_var 的点 ===
# 找出 new_var > old_var 的位置
mask = Var_EqQ > Var_EqP

# 将这些点展平，避免空数组报错
A_flat = A[mask]
B_flat = B[mask]
new_var_flat = Var_EqQ[mask]
...
plt.savefig("./mois_figs/KL_VR_EqP_normal.pdf", bbox_inches='tight')
plt.show()