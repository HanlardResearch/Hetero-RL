    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.loss_type in ["grpo", "bnpo", "dr_grpo","delta_ln","gmpo" ]:
            ...
            if self.loss_type == "EqP":
                ...
                # pou = (-cppo_per_seq_kl/self.script_args.kl_temp).exp()
                # pou_clip = torch.clamp(pou,0,1)
                # coef_1 = learner_seq_p / ( (1-pou_clip)*E_qQ + pou_clip* sampler_seq_p)
        ...
        if self.loss_type == "gepo+":
            self._metrics[mode]["pou"].append(pou.nanmean().item())
            # self._metrics[mode]["pou_clip"].append(pou_clip.nanmean().item())
