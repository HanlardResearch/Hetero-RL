...
class GRPOScriptArguments(trl.ScriptArguments):
    ...
    use_Psync: bool = field(
        default=False,
        metadata={"help": "Whether enable the dynamic model-sync mode."},
    )
    sync_beta: float = field(
        default=0.2,
        metadata={"help": "use to cal Psync"},
    )
    entropy_bar: float = field(
        default=0.1,
        metadata={"help": "use to cal Psync"},
    )
