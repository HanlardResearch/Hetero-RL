...
export SAVEPATH="/extrahome0/save_dir/GPG/8gpus/GRPO/Qwen3-1.7B"
...
accelerate launch --config_file recipes/accelerate_configs/zero2_8A100s.yaml \
  ...
  src/open_r1/grpo.py --config  recipes/Qwen2.5-Math-7B/grpo/config_simple_rl_math_l35_v3_vllm.yaml --output_dir $SAVEPATH \
  ...
  --per_device_eval_batch_size 16  --per_device_train_batch_size 8 --eval_strategy "steps" --eval_steps 32 --eval_on_start True --seed 2025 \
  --logging_steps 1  --use_vllm True  --log_completions True > $log_path 2>&1