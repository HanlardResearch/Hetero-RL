...
export SAVEPATH="/extrahome0/save_dir/GPG/2gpus/${loss_type}/${formatted_time}/Qwen3-1.7B"
...
accelerate launch --config_file recipes/accelerate_configs/zero2_2A100s.yaml \
  ...
  src/open_r1/online_rl_2.py --config  recipes/Qwen2.5-Math-7B/grpo/config_simple_rl_math_l35_v1_vllm.yaml --output_dir $SAVEPATH \
  ...
  --save_strategy "steps" --save_steps 64000 --log_completions False \
  --wandb_entity "pcl-zh"  --wandb_project "GPG"  --report_to "wandb"   \
  --per_device_eval_batch_size 16  --per_device_train_batch_size 8 --eval_strategy "steps" --eval_steps 64 --eval_on_start False --use_benchmark \
  --logging_steps 1  --use_vllm True --loss_type $loss_type
